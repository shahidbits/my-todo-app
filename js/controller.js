(function (window) {
	...
	function Controller(model, view) {
        ...
        that.view.bind('markItemComplete', function (item) {
            that.markItemComplete(item.id);
        });

        that.view.bind('clearItemsCompleted', function (item) {
            that.clearItemsCompleted(item.id);
        });
	...
	/**
	 * By giving it an ID it'll find the DOM element matching that ID,
	 * mark it 'completed' from the DOM.
	 *
	 * @param {number} id The ID of the item to mark as 'completed' from 
	 * the DOM and storage
	 */
	Controller.prototype.markItemComplete = function (id) {
		var that = this;
		that.model.markItemComplete(id, function () {
			that.view.render('markItemComplete', id);
		});
    ...
    /**
     * Find all the completed items in the DOM,
     * removing it from the DOM and the storage.
     */
    Controller.prototype.clearItemsCompleted = function () {
        var that = this;
        that.model.clearItemsCompleted(function (a, b) {
            that.view.render('clearItemsCompleted', b);
        });
